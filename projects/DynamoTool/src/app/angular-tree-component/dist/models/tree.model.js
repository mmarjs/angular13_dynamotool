...
var TreeModel = /** @class */ (function () {
    ...
    TreeModel.prototype.moveNode = function (node, to) {
        ...
        if (to.parent.data.children !== undefined) {
            if (to.parent.data.children[0].menu !== 'remove') {
                if (node.data.title !== undefined) {
                    if (to.parent.data.children !== undefined) {
                        if (to.parent.data.children[0].menu === 'menuCodes') {
                            node.data.title = node.data.name;
                            node.data.name = node.data.menuname;
                        } else {
                            node.data.name = node.data.title;
                        }
                    }
                }
                if (to.parent.data.name === undefined) {

                } else {

                    if (to.parent.data.type !== 'menutree') {
                        if (node.data.menu === 'menuCodes') {
                            alert("menu codes does not set as menu items");
                            // node.data.title = node.data.name;
                            if (node.treeModel.nodes[0].children !== undefined) {
                                return;
                            } else {
                                node.data.name = node.data.menuname;
                                return;
                        ...
                        } else if (node.data.menu === 'menuItems' && to.parent.data.leafnode) {
                            return;
                        } else if (to.parent.data.menu === 'menuItems' && node.data.menu === 'menuItems') {
                            return;
                        }
                    }
                }
            ...
            if (node.data.menu === 'menuItems' && to.parent.data.leafnode) {
                return;
            } else if (to.parent.data.menu === 'menuItems' && node.data.menu === 'menuItems') {
                return;
            }
